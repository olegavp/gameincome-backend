...
class MainPageResource extends JsonResource
{
    public function toArray($request): array
    {
        ...
        $recommendations->map(function ($value) use ($gameKeys, $softwareKeys) {
            if ($gameKeys === null) {
                $itemKeys = $softwareKeys->where('item_id', $value->id)->where('service_price',
                    $softwareKeys->where('item_id', $value->id)->min('service_price'));
            } else {
                $itemKeys = $gameKeys->where('item_id', $value->id)->where('service_price',
                    $gameKeys->where('item_id', $value->id)->min('service_price'));
            }
        ...
        return [
            ...
            'sellersFeedbacks' => MainPageSellersFeedbacksResource::collection(SellerFeedback::query()->with('user',
                'gameKey.item', 'softwareKey.item')->where('rate', 1)->orderBy(DB::raw('RAND()'))->take(5)->get()),
